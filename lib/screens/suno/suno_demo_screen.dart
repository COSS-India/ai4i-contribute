...
class SunoDemoScreen extends StatefulWidget {
  const SunoDemoScreen({super.key});

  @override
  State<SunoDemoScreen> createState() => _SunoDemoScreenState();
}

class _SunoDemoScreenState extends State<SunoDemoScreen> {
  ...
  Future<void> _loadModuleData() async {
    ...
    try {
      statusModel = await ModuleService.getModuleStatus('suno');
      
      if (statusModel == null) {
        setState(() {
          errorMessage = 'Failed to connect to suno service';
          isLoading = false;
        });
        return;
      }
      ...
      sampleModel = await ModuleService.getModuleSample('suno');
