...
class DemoScreen extends StatefulWidget {
  ...
  const DemoScreen({super.key, required this.module});
...
class _DemoScreenState extends State<DemoScreen> {
  ...
  Future<void> _loadModuleData() async {
    ...
    try {
      print('Loading data for module: ${widget.module}');

      // First check status
      statusModel = await ModuleService.getModuleStatus(widget.module);
      print('Status model: ${statusModel?.module} - ${statusModel?.status}');

      if (statusModel == null) {
        setState(() {
          errorMessage = 'Failed to connect to ${widget.module} service';
          isLoading = false;
        });
        return;
      }
      ...
      sampleModel = await ModuleService.getModuleSample(widget.module);
      print('Sample model loaded: ${sampleModel?.module}');
